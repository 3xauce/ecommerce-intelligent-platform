...
export default function MyShopPage() {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }} className="anim-fade-up">
      ...
      {status === 'loading' && <Skeleton variant="rounded" height={360} />}
                ...
                {products.map((product) => (
                  <TableRow key={product.id} hover sx={{ opacity: product.is_active ? 1 : 0.55 }}>
                    ...
                    <TableCell align="center">
                      <Switch size="small" checked={product.is_active} onChange={() => handleToggleActive(product)} />
                    </TableCell>
                    <TableCell align="right">
                      <Tooltip title="Gérer les images">
                        <IconButton size="small" onClick={() => setImagesDialog({ open: true, productId: product.id })}>
                          <ImageRoundedIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Modifier">
                        <IconButton size="small" onClick={() => setFormDialog({ open: true, product })}>
                          <EditRoundedIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Supprimer">
                        <IconButton size="small" onClick={() => setDeleteDialog(product)} sx={{ color: tokens.rose }}>
